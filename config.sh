...
#export SLACKWEBHOOK="YOUR_APP_URL_GOES HERE"
...
FLUENTBITCHART="0.19.9"            # see https://artifacthub.io/packages/helm/fluent/fluent-bit
KUBEPROMETHEUSSTACKCHART="23.1.1"  # see https://artifacthub.io/packages/helm/prometheus-community/kube-prometheus-stack
...
ENVSUBSTVAR='$HTTPPORT $CLUSTER $APP $GRAFANA_LOCAL_ADMIN_PASS $SLACKWEBHOOK $AMTOOL $AMTOOLCONFIG $VERSION $INFLUXPORT $INFLUXUIPORT $GRAFANA_CLOUD_METRICS_PUBLISHER_PASS $LOKI_METRICS_PUBLISHER_URL $LOKI_METRICS_PUBLISHER_USER $PROM_METRICS_PUBLISHER_URL $PROM_METRICS_PUBLISHER_USER $GRAFANACLOUDPROM $GRAFANACLOUDLOGS'
...
if [[ "`which npx`" == "" ]]; then
  echo "no npx... nodejs not installed? "
  exit 1
fi
